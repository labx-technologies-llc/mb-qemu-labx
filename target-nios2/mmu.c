...
static void mmu_flush_pid(CPUNios2State *env, uint32_t pid) {
  ...
  for (idx = 0; idx < env->mmu.tlb_num_entries; idx++) {
    ...
    if ((entry->tag & (1<<10)) && (!(entry->tag & (1<<11))) && ((entry->tag & ((1<<env->mmu.pid_bits)-1)) == pid)) {
...
void mmu_write(CPUNios2State *env, uint32_t rn, uint32_t v) {
  ...
  switch (rn) {
	...
	env->regs[CR_TLBMISC] = (env->regs[CR_TLBMISC] & ((1<<20)-1)) | (((way+1) & (env->mmu.tlb_num_ways-1)) << 20);
      }
    ...
    case CR_TLBMISC:
      ...
      if ((v&CR_TLBMISC_PID_MASK) != (env->mmu.tlbmisc_wr&CR_TLBMISC_PID_MASK)) {
	mmu_flush_pid(env, (env->mmu.tlbmisc_wr&CR_TLBMISC_PID_MASK) >> 4);
      }
      ...
      if (v & CR_TLBMISC_RD) {
        ...
        env->regs[CR_TLBMISC] = (v & ~CR_TLBMISC_PID_MASK);
        env->regs[CR_TLBMISC] |= (entry->tag & ((1<<env->mmu.pid_bits)-1)) << 4;
        env->regs[CR_PTEADDR] &= ~CR_PTEADDR_VPN_MASK;
        env->regs[CR_PTEADDR] |= (entry->tag >> 12) << 2;
	MMU_LOG(qemu_log("TLB READ way %d, vpn %05X, tag %08X, data %08X, tlbacc %08X, tlbmisc %08X, pteaddr %08X\n",
                         way, vpn, entry->tag, entry->data, env->regs[CR_TLBACC], env->regs[CR_TLBMISC], env->regs[CR_PTEADDR]));
      } else {
    ...
    case CR_PTEADDR:
      ...
      env->regs[CR_PTEADDR] = (v & ~CR_PTEADDR_VPN_MASK) | (env->regs[CR_PTEADDR] & CR_PTEADDR_VPN_MASK);
